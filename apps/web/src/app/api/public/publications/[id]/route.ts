...
import { shouldUseCachedTranslation } from '@/lib/translations';
...
export async function GET(req: Request, { params }: Params) {
  ...
  if (locale !== 'en') {
    ...
    if (shouldUseCachedTranslation(cached)) {
      ...
      // Translations are pre-generated by the worker at digest publish time.
      // Cache miss: serve English + machine-translation fallback link (no AI spend).
  ...
  return NextResponse.json({
    ...
    originalUrl: sanitizeHttpUrl(publication.originalUrl) ?? '',
    importance: publication.importance ?? null,
    locale: resolvedLocale,
    summary,
    translationFallbackUrl: sanitizeHttpUrl(translationFallbackUrl),
  });
}
